...
function LogIn() {
    ...
    const handleSubmit = () => {
        setErrorMail('');
        setErrorPassword('');
        if (!regexEmail.test(email)) setErrorMail('Por favor, ingresa un correo electrónico válido.');
        if (email.length === 0) setErrorMail('Por favor, ingresa un correo electrónico.');
        if (password.length < 6) setErrorPassword('La contraseña debe tener al menos 6 caracteres.');
        if (password.length === 0) setErrorPassword('Por favor, ingresa una contraseña.');
